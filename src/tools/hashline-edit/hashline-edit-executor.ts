...
export async function executeHashlineEditTool(
  ...
  try {
    ...
    // Return metadata directly via context.metadata if available
    if (typeof metadataContext.metadata === 'function') {
      metadataContext.metadata(meta);
    }

    // In medium, we don't have the storeToolMetadata feature - metadata is returned via context.metadata
    // If callID is available and there's a way to store, it would go here
    // For medium, we rely on the metadata callback instead
    const _callID = resolveToolCallID(metadataContext);
    void _callID; // suppress unused variable warning - kept for potential future use
