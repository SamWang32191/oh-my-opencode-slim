...
1. Install with Antigravity support:
   ```bash
   bunx oh-my-opencode-medium install --antigravity=yes
   ```
...
If you prefer to configure manually, edit `~/.config/opencode/oh-my-opencode-medium.json` (or `.jsonc`) and add a pure Antigravity preset:
...
# Reinstall plugin
bunx oh-my-opencode-medium install --antigravity=yes --no-tui --kimi=no --openai=no --tmux=no --skills=no
```

### Wrong Model Selected
```bash
# Check current preset
echo $OH_MY_OPENCODE_MEDIUM_PRESET

# Change preset
export OH_MY_OPENCODE_MEDIUM_PRESET=antigravity
opencode
```
...
# Or edit ~/.config/opencode/oh-my-opencode-medium.json (or .jsonc)
